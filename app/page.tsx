...
export default function LandingPage() {
  ...
  const handleLogin = async (formData: FormData) => {
    ...
    if (res?.error) {
      ...
      if (res.error.includes("Invalid login")) {
       ...
       // Refresh router agar cookie terbaca
       router.refresh(); 
       
       // Redirect Client Side (Lebih aman untuk cookie)
          ...
          {user ? (
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-6 text-center space-y-4 animate-in zoom-in-95">
               ...
               <button 
                 ...
                 className="block w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-md transition-all flex items-center justify-center gap-2 hover:shadow-lg active:scale-95"
               >
            ...
            <form action={handleLogin} className="space-y-6">
                <div className="space-y-2">
                    ...
                    <div className="relative group">
                        ...
                        <input 
                          ...
                          className="block w-full pl-10 pr-3 py-3 border border-slate-300 rounded-xl bg-slate-50 text-slate-900 focus:ring-2 focus:ring-green-500 outline-none transition-all shadow-sm disabled:opacity-50 disabled:bg-slate-100" 
                        />
                    </div>
                </div>
                ...
                <button 
                  type="submit"
                  disabled={isSubmitting}
                  className="w-full flex justify-center items-center gap-2 py-3.5 px-4 border border-transparent rounded-xl shadow-lg text-sm font-bold text-white bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-2 focus:ring-green-500 transition-all transform active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none"
                >
