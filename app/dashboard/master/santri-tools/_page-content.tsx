...
const STATUS_COLOR: Record<string, string> = {
  ...
  tidak_diketahui: 'bg-gray-100 text-gray-500',
}
  ...
  return (
    <div className="max-w-5xl mx-auto pb-20 space-y-6">
      ...
      <div className="border-b pb-4">
        ...
        <p className="text-sm text-gray-500 mt-1">Operasi massal data santri — naik kelas sekolah & manajemen pembebasan.</p>
      </div>
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit">
        <button onClick={() => setTab('naik_kelas')}
          className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-bold transition-all ${
            tab === 'naik_kelas' ? 'bg-white shadow text-indigo-700' : 'text-gray-500 hover:text-gray-700'
          }`}>
          <ArrowUpCircle className="w-4 h-4"/> Naik Kelas Sekolah
        </button>
        <button onClick={() => setTab('pembebasan')}
          className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-bold transition-all ${
            tab === 'pembebasan' ? 'bg-white shadow text-indigo-700' : 'text-gray-500 hover:text-gray-700'
          }`}>
      ...
      {tab === 'naik_kelas' && (
        <div className="space-y-5">
          ...
          <div className="bg-white border rounded-xl p-4 flex flex-wrap gap-3 items-end">
            ...
            <div>
              <label className="text-xs font-bold text-gray-500 block mb-1">Filter Kelas (opsional)</label>
              <input value={filterKelas} onChange={e => setFilterKelas(e.target.value)}
                placeholder="mis: 7, 8A"
                className="border rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500 w-32"/>
            </div>
                    ...
                    <div className="divide-y max-h-64 overflow-y-auto">
                      {list.map(s => (
                        <div key={s.id} onClick={() => toggleSelect(s.id)}
                          className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer hover:bg-gray-50 transition-colors ${selectedIds.has(s.id) ? 'bg-indigo-50' : ''}`}>
                          {selectedIds.has(s.id)
                            ? <CheckSquare className="w-4 h-4 text-indigo-600 shrink-0"/>
                            : <Square className="w-4 h-4 text-gray-300 shrink-0"/>}
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-gray-800 truncate">{s.nama_lengkap}</p>
                            <p className="text-[10px] text-gray-400">{s.nis} · {s.asrama} · {s.sekolah || '-'}</p>
                          </div>
                          <div className="flex items-center gap-2 shrink-0 text-sm">
                            <span className="font-mono bg-gray-100 px-2 py-0.5 rounded text-gray-700">{s.kelas_sekolah}</span>
              ...
              {grouped.tidak_diketahui.length > 0 && (
                <div className="bg-white border rounded-xl overflow-hidden">
                  <button onClick={() => setShowTidakDikenal(v => !v)}
                    ...
                    {showTidakDikenal ? <ChevronUp className="w-4 h-4 text-gray-400"/> : <ChevronDown className="w-4 h-4 text-gray-400"/>}
                  </button>
                  {showTidakDikenal && (
                    <div className="divide-y max-h-48 overflow-y-auto">
                      {grouped.tidak_diketahui.map(s => (
                        <div key={s.id} className="flex items-center gap-3 px-4 py-2.5">
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-gray-700 truncate">{s.nama_lengkap}</p>
                            <p className="text-[10px] text-gray-400">{s.nis} · {s.asrama}</p>
                          </div>
                          <span className="font-mono bg-gray-100 text-gray-500 px-2 py-0.5 rounded text-xs">"{s.kelas_sekolah}"</span>
                        </div>
                      ))}
                    </div>
                  )}
      ...
      {tab === 'pembebasan' && (
        <div className="space-y-5">
          ...
          <div className="bg-white border rounded-xl p-4 flex flex-wrap gap-3 items-end">
            ...
            <div>
              <label className="text-xs font-bold text-gray-500 block mb-1">Cari Nama / NIS</label>
              <input value={pbSearch} onChange={e => setPbSearch(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && loadPembebasan()}
                placeholder="Nama atau NIS..."
                className="border rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500 w-44"/>
            </div>
            <div>
              <label className="text-xs font-bold text-gray-500 block mb-1">Tahun Pembayaran</label>
              <div className="flex items-center border rounded-lg overflow-hidden">
                <button onClick={() => setPbTahun(t => t - 1)} className="px-3 py-2 hover:bg-gray-100 text-sm font-bold">-</button>
            ...
            <label className="flex items-center gap-2 cursor-pointer select-none">
              ...
              <span className="text-sm font-semibold text-gray-600">Hanya yg bebas SPP</span>
            </label>
          ...
          {pbSelected.size > 0 && (
            <div className="flex items-center gap-3 bg-indigo-50 border border-indigo-200 rounded-xl px-4 py-3">
              ...
              <button onClick={() => setPbSelected(new Set())} className="ml-auto text-xs text-gray-400 hover:text-gray-600">Batal pilih</button>
            </div>
          )}
          ...
          {pbLoading ? (
            ...
            <div className="flex flex-col items-center py-20 gap-3 text-center">
              ...
              <p className="text-gray-400 font-semibold">Belum ada data</p>
              <p className="text-sm text-gray-400">Atur filter lalu tekan Tampilkan.</p>
            </div>
          ) : (
            <div className="bg-white border rounded-xl overflow-hidden">
              <div className="bg-gray-50 px-4 py-2.5 border-b flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-gray-500 uppercase tracking-wide">{pbData.length} Santri</span>
                </div>
                <span className="text-xs text-gray-400">Klik nama untuk kelola pembayaran tahunan</span>
              </div>
              <div className="divide-y max-h-[500px] overflow-y-auto">
                {pbData.map(s => (
                  <div key={s.id} className={`flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors ${pbSelected.has(s.id) ? 'bg-indigo-50' : ''}`}>
                    ...
                    <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setModalSantri(s)}>
                      <p className="text-sm font-bold text-gray-800 truncate">{s.nama_lengkap}</p>
                      <p className="text-[10px] text-gray-400">{s.nis} · {s.asrama} Kamar {s.kamar}</p>
                    </div>
                    {/* Badge bebas SPP */}
                    <button
            ...
            <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setModalSantri(null)}>
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                <div className="px-5 py-4 border-b flex items-center justify-between">
                  <div>
                    <h3 className="font-bold text-gray-800">{modalSantri.nama_lengkap}</h3>
                    <p className="text-xs text-gray-500">{modalSantri.nis} · Tahun {pbTahun}</p>
                  </div>
                  <button onClick={() => setModalSantri(null)} className="p-1.5 hover:bg-gray-100 rounded-full">
                    <X className="w-5 h-5 text-gray-400"/>
                  </button>
                </div>
                <div className="p-5 space-y-3">
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-wide">Pembayaran Tahunan — Tahun {pbTahun}</p>
                  {JENIS_BIAYA_LIST.map(jenis => {
                    const sudahBebas = modalSantri.sudah_bayar.includes(jenis)
                    return (
                      <div key={jenis} className={`flex items-center justify-between p-3 rounded-xl border ${sudahBebas ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-200'}`}>
                        <div className="flex items-center gap-2">
                          <Banknote className={`w-4 h-4 ${sudahBebas ? 'text-blue-500' : 'text-gray-400'}`}/>
                ...
                <div className="px-5 pb-5">
                  <p className="text-[10px] text-gray-400">Pembebasan dicatat sebagai transaksi Rp 0. Hanya admin yang dapat mengubah ini.</p>
                </div>
              </div>
            </div>
          )}
