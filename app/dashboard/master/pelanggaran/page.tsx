    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="bg-white p-6 rounded-xl border shadow-sm">
        ...
        <form action={async (formData) => {
          ...
          <button className="bg-green-700 text-white p-2 rounded w-full md:w-auto hover:bg-green-800">Simpan</button>
        </form>
      </div>
            ...
            {list.map((item: any) => (
              <tr key={item.id} className="hover:bg-gray-50">
                <td className="px-6 py-3">
                  <span className={`px-2 py-1 rounded text-xs font-bold ${item.kategori === 'BERAT' ? 'bg-red-100 text-red-700' : item.kategori === 'SEDANG' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>
                    {item.kategori}
                  </span>
                </td>
