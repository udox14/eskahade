...
import { CheckCircle, Landmark, Loader2, RotateCcw } from 'lucide-react'
...
export default function PageContent() {
  ...
  return <div className="space-y-5 p-6">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="flex items-center gap-2 text-2xl font-bold"><Landmark className="h-6 w-6 text-emerald-700"/>Setoran SPP Santri Baru</h1><p className="text-sm text-slate-500">Penerimaan pusat untuk tagihan SPP Juli santri angkatan PSB.</p></div><div className="flex gap-2"><select value={bulan} onChange={e => setBulan(Number(e.target.value))} className="rounded-lg border px-3 py-2">{Array.from({length:12},(_,i)=><option key={i+1} value={i+1}>{i+1}</option>)}</select><input type="number" value={tahun} onChange={e=>setTahun(Number(e.target.value))} className="w-24 rounded-lg border px-3 py-2"/></div></div>
