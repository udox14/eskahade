...
export default async function CentralFinancePage() {
  ...
  return <main className="space-y-4 sm:space-y-5">
    <FinancePageHeader title="Keuangan Terpusat" description="Pantau posisi dana, alokasi santri, dan pekerjaan yang perlu ditindaklanjuti." eyebrow="Ledger terpisah dari legacy" meta={`${scope ? `Scope asrama: ${scope}` : 'Scope global'} · pembaruan terakhir berdasarkan jurnal terposting`} />
    <FinanceNav />
    ...
    <section className="grid gap-4 xl:grid-cols-[1.35fr_.9fr]">
      <SectionPanel title="Tren arus kas 30 hari" description="Debit dan kredit pada rekening bank/clearing; untuk membaca pola, bukan menggantikan rekonsiliasi.">
        <div className="p-4">
          {data.cashTrend.length ? <>
            <div className="mb-3 flex gap-4 text-[11px] sm:text-xs"><span className="flex items-center gap-1 text-emerald-700"><ArrowDownToLine className="h-3.5 w-3.5"/>Dana masuk</span><span className="flex items-center gap-1 text-blue-700"><ArrowUpFromLine className="h-3.5 w-3.5"/>Dana keluar</span></div>
            ...
            <div className="mt-2 flex justify-between text-[10px] text-slate-400"><span>{data.cashTrend[0]?.day}</span><span>{data.cashTrend.at(-1)?.day}</span></div>
          </> : <div className="grid h-44 place-items-center text-center text-sm text-slate-500"><p>Belum ada arus kas terposting dalam 30 hari terakhir.<small className="mt-1 block">Grafik akan terisi otomatis setelah transaksi pertama.</small></p></div>}
        </div>
      </SectionPanel>

      <SectionPanel title="Komposisi alokasi wallet" description="Membantu melihat konsentrasi dana santri per kebutuhan.">
        <div className="space-y-3 p-4">
          {data.walletTotals.length ? data.walletTotals.map(row => <div key={row.wallet_kind}>
            <div className="mb-1 flex items-center justify-between gap-3 text-xs"><span className="font-semibold text-slate-700">{row.wallet_kind}</span><span className="font-bold tabular-nums">{rupiah(Number(row.balance_rupiah))}</span></div>
            <div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-emerald-600" style={{width:`${Number(row.balance_rupiah)/maxWallet*100}%`}} /></div>
          </div>) : <p className="py-16 text-center text-sm text-slate-500">Belum ada dana yang dialokasikan.</p>}
        </div>
      </SectionPanel>
    </section>

    <section className="grid gap-4 xl:grid-cols-[.8fr_1.2fr]">
      <SectionPanel title="Pengecualian butuh tindakan" description="Kerjakan dari dampak tertinggi; jangan menyembunyikan selisih.">
        <div className="divide-y divide-slate-100">
          {data.alerts.map(row => <div key={row.kind} className="flex items-center justify-between gap-3 px-4 py-3 text-xs sm:gap-4 sm:text-sm">
            ...
            <div className="text-right"><StatusBadge tone={Number(row.count)?'amber':'emerald'}>{row.count} kasus</StatusBadge><p className="mt-1 text-xs tabular-nums text-slate-500">{rupiah(Number(row.amount_rupiah))}</p></div>
          </div>)}
        </div>
      </SectionPanel>

      <SectionPanel title="Aktivitas jurnal terbaru" description="Urutan transaksi berdasarkan waktu posting paling baru.">
    ...
    <details className="overflow-hidden rounded-xl border border-slate-200 bg-white">
      <summary className="cursor-pointer px-4 py-3 text-sm font-bold text-slate-800">Chart of accounts <span className="ml-1 font-normal text-slate-500">({data.accountBalances.length} akun)</span></summary>
      <div className="divide-y divide-slate-100 border-t border-slate-100 sm:hidden">{data.accountBalances.map(row => <article key={row.code} className="flex items-start justify-between gap-3 px-4 py-3 text-xs"><div className="min-w-0"><p className="font-mono text-[10px] text-slate-400">{row.code} · {row.account_type}</p><p className="mt-0.5 font-semibold text-slate-800">{row.name}</p></div><strong className="shrink-0 text-right tabular-nums text-slate-900">{rupiah(Number(row.balance_rupiah))}</strong></article>)}</div>
