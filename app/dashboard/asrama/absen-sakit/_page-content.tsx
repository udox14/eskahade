  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto pb-20">
      ...
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold text-slate-800">Absen Sakit Pagi</h1>
        <p className="text-slate-500 text-sm">Input data santri sakit & status surat dokter.</p>
        ...
        <div className={`p-2 rounded-lg border inline-flex items-center gap-2 w-fit mt-2 ${userAsrama ? 'bg-green-50 border-green-200' : 'bg-white'}`}>
          ...
          <select 
            ...
            disabled={!!userAsrama}
            className={`bg-transparent text-sm font-bold text-slate-700 outline-none ${userAsrama ? 'cursor-not-allowed opacity-80' : 'cursor-pointer'}`}
          >
      ...
      <div className="bg-white p-5 rounded-xl border shadow-sm space-y-4">
        <label className="text-xs font-bold text-slate-500 uppercase">Input Santri Sakit Hari Ini</label>
        ...
        {hasilCari.length > 0 && (
          <div className="space-y-2 mt-2 animate-in fade-in slide-in-from-top-2">
            {hasilCari.map(s => (
              <div key={s.id} className="p-3 border border-slate-200 rounded-xl bg-blue-50/50 flex flex-col sm:flex-row justify-between items-center gap-3">
                <div className="text-center sm:text-left">
                  <p className="font-bold text-slate-800">{s.nama_lengkap}</p>
                  <p className="text-xs text-slate-500">Kamar {s.kamar}</p>
                </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-slate-700 text-sm">Daftar Sakit Minggu Ini</h3>
          <span className="text-xs bg-slate-100 text-slate-500 px-2 py-1 rounded">Reset tiap Senin</span>
        </div>

        {loadingList ? (
          <div className="text-center py-8"><Loader2 className="w-6 h-6 animate-spin mx-auto text-slate-400"/></div>
        ) : listSakit.length === 0 ? (
          <div className="text-center py-10 border-2 border-dashed rounded-xl text-slate-400 text-sm">
          ...
          listSakit.map((item) => (
            <div key={item.id} className="bg-white p-4 rounded-xl border flex justify-between items-center group hover:shadow-sm transition-shadow">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-slate-800 text-sm">{item.santri?.nama_lengkap}</p>
                  <div className="flex items-center gap-2 text-xs mt-0.5">
                    <span className="text-slate-500">{format(new Date(item.tanggal), 'EEEE, dd MMM', { locale: id })}</span>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden transform transition-all scale-100">
            <div className="p-6 text-center">
              ...
              <h3 className="text-lg font-bold text-slate-800 mb-1">{confirmState.message}</h3>
              <p className="text-sm text-slate-500 mb-6">{confirmState.description}</p>
              
              <div className="grid grid-cols-2 gap-3">
                <button 
                  onClick={() => setConfirmState(prev => ({ ...prev, isOpen: false }))}
                  className="py-2.5 rounded-xl border border-slate-300 text-slate-600 font-bold hover:bg-slate-50 transition-colors"
                >
                  Batal
                </button>
                <button 
                  onClick={confirmState.onConfirm}
                  className={`py-2.5 rounded-xl text-white font-bold shadow-sm transition-colors ${
