                ...
                <form onSubmit={handleTopup}>
                  <div className="flex flex-col gap-2 sm:flex-row">
                    <input type="number" placeholder="Nominal Rp..."
                      className="min-w-0 flex-1 p-2 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500"
                      value={topupNominal}
                      onChange={e => setTopupNominal(e.target.value)}
                    />
                    <button disabled={isSaving} className="w-full shrink-0 bg-emerald-600 text-white px-4 py-2 rounded-lg font-bold hover:bg-emerald-700 text-sm sm:w-auto">
                      {isSaving ? '...' : 'Simpan'}
                    </button>
                  </div>
                </form>
