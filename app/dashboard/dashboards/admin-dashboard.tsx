...
export async function AdminDashboard() {
  ...
  const { total: totalGuru } = await queryOne<{ total: number }>(
    'SELECT COUNT(*) AS total FROM data_guru', []
  ) ?? { total: 0 }

  const sppRows = await query<{ nominal_bayar: number }>(
    'SELECT nominal_bayar FROM spp_log WHERE tahun = ?', [currentYear]
  )
  const totalPemasukanSPP = sppRows.reduce((sum, item) => sum + item.nominal_bayar, 0)

  const { total: izinAktif } = await queryOne<{ total: number }>(
    "SELECT COUNT(*) AS total FROM perizinan WHERE status = 'AKTIF'", []
  ) ?? { total: 0 }

  const { total: sakitHariIni } = await queryOne<{ total: number }>(
    'SELECT COUNT(*) AS total FROM absen_sakit WHERE tanggal = ?', [today]
  ) ?? { total: 0 }

  const { total: pelanggaranHariIni } = await queryOne<{ total: number }>(
    'SELECT COUNT(*) AS total FROM pelanggaran WHERE tanggal = ?', [today]
  ) ?? { total: 0 }

  const tunggakanSPP = await getRingkasanTunggakan()

  const recentViolations = await query<any>(`
    SELECT p.*, s.nama_lengkap, s.asrama, s.kamar
    FROM pelanggaran p
    JOIN santri s ON s.id = p.santri_id
    ORDER BY p.created_at DESC LIMIT 4
  `, [])

  const recentPermissions = await query<any>(`
    SELECT p.*, s.nama_lengkap, s.asrama
    FROM perizinan p
    JOIN santri s ON s.id = p.santri_id
    ORDER BY p.created_at DESC LIMIT 4
  `, [])
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 pb-10">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          <StatCard label="Total Santri Aktif" value={totalSantri} icon={Users} color="bg-blue-600" textColor="text-blue-600" bgLight="bg-blue-50" borderHover="hover:border-blue-300" href="/dashboard/santri" />
          <StatCard label="Tenaga Pengajar" value={totalGuru} icon={Briefcase} color="bg-purple-600" textColor="text-purple-600" bgLight="bg-purple-50" borderHover="hover:border-purple-300" href="/dashboard/master/wali-kelas" />
          <StatCard label="Rombongan Belajar" value={totalKelas} icon={School} color="bg-indigo-600" textColor="text-indigo-600" bgLight="bg-indigo-50" borderHover="hover:border-indigo-300" href="/dashboard/master/kelas" />
          <StatCard label={`Pemasukan SPP ${currentYear}`} value={`Rp ${(totalPemasukanSPP / 1000000).toFixed(1)}Jt`} icon={Coins} color="bg-emerald-600" textColor="text-emerald-600" bgLight="bg-emerald-50" borderHover="hover:border-emerald-300" href="/dashboard/keuangan/laporan" />
        </div>
      </div>

      {/* 2. MENU AKSES CEPAT ADMIN */}
      <div className="bg-white rounded-3xl p-6 md:p-8 border border-slate-200 shadow-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <QuickAction href="/dashboard/pengaturan/users" icon={UserCog} title="Manajemen User" desc="Hak Akses & Sandi" theme="blue" />
            <QuickAction href="/dashboard/master/kelas" icon={Database} title="Master Kelas" desc="Tahun Ajaran & Kelas" theme="indigo" />
            <QuickAction href="/dashboard/master/wali-kelas" icon={Briefcase} title="Master Guru" desc="Pengajar & Jadwal" theme="purple" />
            <QuickAction href="/dashboard/master/kitab" icon={Book} title="Master Kitab" desc="Buku & Harga UPK" theme="emerald" />
            <QuickAction href="/dashboard/master/pelanggaran" icon={Settings} title="Aturan Pelanggaran" desc="Poin & Kategori Kasus" theme="rose" />
            <QuickAction href="/dashboard/keuangan/tarif" icon={Wallet} title="Pengaturan Tarif" desc="Biaya Angkatan Masuk" theme="amber" />
            <QuickAction href="/dashboard/dewan-santri/sensus" icon={BarChart3} title="Sensus Penduduk" desc="Statistik Demografi" theme="cyan" />
            <QuickAction href="/dashboard/keuangan/laporan" icon={FileText} title="Laporan Keuangan" desc="Arus Kas & Tunggakan" theme="teal" />
        </div>
      </div>

      {/* 3. PANTAUAN OPERASIONAL */}
      <div>
        <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-4 px-1 flex items-center gap-2">
            <Activity className="w-4 h-4"/> Pantauan Operasional Harian
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <StatCard label="Sedang Izin" value={izinAktif} icon={MapPin} color="bg-sky-600" textColor="text-sky-600" bgLight="bg-sky-50" borderHover="hover:border-sky-300" href="/dashboard/keamanan/perizinan" />
          <StatCard label="Sakit Hari Ini" value={sakitHariIni} icon={Stethoscope} color="bg-orange-500" textColor="text-orange-600" bgLight="bg-orange-50" borderHover="hover:border-orange-300" href="/dashboard/asrama/absen-sakit" />
          <StatCard label="Pelanggaran Hari Ini" value={pelanggaranHariIni} icon={ShieldAlert} color="bg-red-600" textColor="text-red-600" bgLight="bg-red-50" borderHover="hover:border-red-300" href="/dashboard/keamanan" />
          <StatCard label="Penunggak SPP" value={tunggakanSPP} icon={Receipt} color="bg-rose-600" textColor="text-rose-600" bgLight="bg-rose-50" borderHover="hover:border-rose-300" href="/dashboard/asrama/spp" subLabel="Peringatan Admin" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/80">
              <h3 className="font-bold text-slate-800 flex items-center gap-2"><Clock className="w-4 h-4 text-sky-500"/> Izin Keluar Terbaru</h3>
                ...
                recentPermissions.map((p: any) => (
                  <div key={p.id} className="p-3 mx-2 my-1 rounded-xl flex items-center justify-between hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${p.jenis === 'PULANG' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>
                        {p.nama_lengkap?.substring(0, 1)}
                      </div>
                      <div>
                    ...
                    <span className={`text-[10px] px-2.5 py-1 rounded-full font-bold border ${p.status === 'AKTIF' ? 'bg-yellow-50 text-yellow-700 border-yellow-200' : 'bg-green-50 text-green-700 border-green-200'}`}>
                      {p.status}
                    </span>
                  </div>
                ))
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col h-full">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-red-50/40">
              <h3 className="font-bold text-slate-800 flex items-center gap-2"><AlertTriangle className="w-4 h-4 text-red-500"/> Pelanggaran Terkini</h3>
                ...
                recentViolations.map((v: any) => (
                  <div key={v.id} className="p-4 rounded-xl bg-slate-50 border border-slate-100 hover:bg-white hover:shadow-md transition-all group">
                    <div className="flex justify-between items-start mb-2">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${v.jenis === 'BERAT' ? 'bg-red-100 text-red-700 border-red-200' : v.jenis === 'SEDANG' ? 'bg-orange-100 text-orange-700 border-orange-200' : 'bg-slate-200 text-slate-600 border-slate-300'}`}>{v.jenis}</span>
                      <span className="text-[10px] text-slate-400 font-mono">{new Date(v.created_at).toLocaleDateString('id-ID')}</span>
                    </div>
                    <p className="text-sm font-bold text-slate-800 mb-0.5 group-hover:text-blue-600 transition-colors">{v.nama_lengkap}</p>
                    <p className="text-xs text-slate-500 mb-2">{v.asrama} - Kamar {v.kamar}</p>
                    <p className="text-xs text-slate-600 leading-relaxed bg-white p-2 rounded border border-slate-100 line-clamp-2">{v.deskripsi}</p>
                  </div>
                ))
              )}
            </div>
    ...
    <Link href={href} className="block group h-full">
      <div className={`bg-white p-4 lg:p-5 rounded-2xl border border-slate-200 shadow-sm transition-all duration-300 hover:-translate-y-1 h-full flex flex-col justify-between relative overflow-hidden ${borderHover}`}>
        ...
        <div className="z-10 relative">
          ...
          <h4 className={`text-2xl lg:text-3xl font-extrabold tracking-tight ${String(value).includes('Rp') ? 'text-slate-700' : 'text-slate-800'}`}>{value ?? 0}</h4>
          {subLabel && <p className={`text-[10px] font-bold ${textColor} mt-2 bg-white/80 inline-block px-2 py-0.5 rounded border`}>{subLabel}</p>}
        </div>
      </div>
    </Link>
  )
...
function QuickAction({ href, icon: Icon, title, desc, theme = 'blue' }: any) {
  return (
    <Link href={href} className={`group flex flex-col items-center justify-center p-5 rounded-2xl border transition-all duration-300 hover:-translate-y-1 hover:shadow-lg cursor-pointer ${themes[theme]}`}>
