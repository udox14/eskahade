...
export async function KeamananDashboard() {
  ...
  const { total: pelanggaranToday } = await queryOne<{ total: number }>(
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
      <div className="bg-gradient-to-r from-red-800 to-rose-900 rounded-2xl p-8 text-white shadow-lg relative overflow-hidden">
        <div className="relative z-10">
          ...
          <h1 className="text-3xl font-extrabold flex items-center gap-3"><ShieldAlert className="w-8 h-8"/> Keamanan & Disiplin</h1>
          <p className="mt-2 text-red-100 max-w-lg">Penegakan disiplin, monitoring keterlambatan, dan penindakan pelanggaran.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
          ...
          <div className="p-4 rounded-full bg-red-50 text-red-600"><AlertTriangle className="w-8 h-8"/></div>
        </div>
        ...
        <Link href="/dashboard/keamanan/perizinan/verifikasi-telat" className="block group">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between hover:shadow-md hover:border-orange-300 transition-all cursor-pointer">
            ...
            <div className="p-4 rounded-full bg-orange-50 text-orange-600 group-hover:scale-110 transition-transform"><Clock className="w-8 h-8"/></div>
          </div>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <ActionCard href="/dashboard/keamanan/input" title="Input Pelanggaran" desc="Catat kasus indisipliner santri." icon={Plus} color="bg-red-600" />
            <ActionCard href="/dashboard/keamanan/perizinan/verifikasi-telat" title="Sidang Keterlambatan" desc="Proses/Vonis santri yang telat." icon={Clock} color="bg-orange-600" />
            <ActionCard href="/dashboard/keamanan/perizinan/cetak-telat" title="Cetak Daftar Panggil" desc="List santri telat untuk Mading." icon={ShieldAlert} color="bg-slate-700" />
          </div>
        </div>
            ...
            {recent.length === 0 ? (
              ...
              recent.map((v: any) => (
                <div key={v.id} className="flex gap-3 items-start">
                  ...
                  <div>
                    <p className="text-sm font-bold text-slate-800">{v.nama_lengkap}</p>
                    <p className="text-[10px] text-slate-500 font-mono mb-1">{v.asrama} - {v.kamar}</p>
