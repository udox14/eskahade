    ...
    <div className="flex flex-col gap-3">
      <div className="rounded-2xl bg-slate-900 p-4 text-white">
        ...
        <div className="mt-1 flex items-center justify-between">
          <span className="text-sm text-white/70">Diterima</span>
          <span className="text-2xl font-extrabold tabular-nums">{rupiah(bayar)}</span>
        </div>
      </div>

      {diff > 0 && (
        <div className="rounded-xl bg-green-50 px-3 py-2 text-sm font-bold text-green-700">
          Kembalian: {rupiah(diff)}
        </div>
      )}
      {diff < 0 && (
        <div className="rounded-xl bg-red-50 px-3 py-2 text-sm font-bold text-red-700">
          Tunggakan: {rupiah(Math.abs(diff))}
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-2">
        {QUICK.map((v) => (
          <button
            ...
            className="rounded-xl border border-slate-200 bg-white py-2.5 text-sm font-bold text-slate-700 transition active:scale-95 hover:border-green-300"
          >
        ...
        <button
          ...
          className="rounded-xl border border-green-200 bg-green-50 py-2.5 text-sm font-bold text-green-700 transition active:scale-95"
        >
          Uang Pas
        </button>
      </div>
...
export function BayarSheet({ open, onClose, ...rest }: BayarProps & { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end xl:hidden">
      ...
      <div className="relative z-10 max-h-[92vh] w-full overflow-y-auto rounded-t-3xl bg-white p-4 pb-[calc(env(safe-area-inset-bottom)+1rem)] shadow-2xl animate-in slide-in-from-bottom duration-300">
...
export function BayarPanel(props: BayarProps & { hasAntrian: boolean }) {
  ...
  return (
    <section className={cn('rounded-2xl border border-slate-200 bg-white p-4', !hasAntrian && 'opacity-50')}>
      <h2 className="mb-3 font-bold text-slate-800">Pembayaran</h2>
