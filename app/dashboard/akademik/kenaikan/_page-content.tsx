        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-left-2">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-end gap-4">
            <div className="w-full md:w-2/5">
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Tingkat / Marhalah Asal</label>
              <select 
                className="w-full p-3 border border-slate-300 rounded-xl bg-slate-50 focus:ring-2 focus:ring-indigo-500 font-medium text-sm outline-none transition-all"
            ...
            <div className="w-full md:w-2/5">
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Kelas Asal (Opsional)</label>
              <select 
                className="w-full p-3 border border-slate-300 rounded-xl bg-slate-50 focus:ring-2 focus:ring-indigo-500 font-medium text-sm outline-none transition-all disabled:opacity-50"
                value={selectedKelas}
            ...
            <button 
              ...
              className="w-full md:w-1/5 bg-slate-800 hover:bg-slate-900 text-white py-3 rounded-xl font-bold shadow-sm transition-colors disabled:opacity-50 flex justify-center items-center gap-2"
            >
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              <div className="bg-indigo-50/50 p-4 border-b border-indigo-100 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-2 w-full lg:w-auto bg-white p-2 rounded-xl border border-indigo-100 shadow-sm">
                  <select 
                    className="w-full sm:w-auto p-2 text-sm border-r border-slate-200 outline-none text-slate-700 font-medium bg-transparent"
              ...
              <div className="overflow-x-auto max-h-[600px]">
                <table className="w-full text-sm text-left relative">
                  <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold uppercase text-xs sticky top-0 z-10 shadow-sm">
                    <tr>
                      ...
                      <th className="p-4 text-center">Grade Rekomendasi<br/><span className="text-[9px] text-slate-400 normal-case">(Dari Wali Kelas)</span></th>
                      <th className="p-4 bg-indigo-50 border-l border-indigo-100 w-64">Target Kelas Baru</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {manualSantri.map(s => (
                      <tr key={s.nis} className={`hover:bg-slate-50 transition-colors cursor-pointer ${selectedNis.includes(s.nis) ? 'bg-indigo-50/30' : ''}`} onClick={() => toggleSelectNis(s.nis)}>
                        <td className="p-4 text-center">
                          {selectedNis.includes(s.nis) ? <CheckSquare className="w-5 h-5 text-indigo-600 mx-auto"/> : <Square className="w-5 h-5 text-slate-300 mx-auto"/>}
                        </td>
                        <td className="p-4">
                          <p className="font-bold text-slate-800 text-base">{s.nama}</p>
                        ...
                        <td className="p-4 bg-indigo-50/20 border-l border-indigo-50" onClick={e => e.stopPropagation()}>
                           ...
                           {targetKelasList.length > 0 ? (
                             <select 
                               className={`w-full p-2.5 border border-slate-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-indigo-500 outline-none transition-all ${
                                 placements[s.nis] ? 'border-indigo-300 bg-indigo-50 text-indigo-800 shadow-inner' : 'border-slate-300 bg-white text-slate-700'
                               }`}
                             ...
                             <div className="text-xs text-slate-400 italic bg-white p-2.5 border border-slate-200 rounded-xl text-center">
          ...
          {totalPlacementsReady > 0 && (
            <div className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[95%] md:w-auto md:min-w-[400px] bg-slate-900 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center justify-between z-40 animate-in slide-in-from-bottom-5">
              ...
              <button 
                ...
                className="bg-emerald-500 hover:bg-emerald-400 text-slate-900 px-5 py-2.5 rounded-xl font-black flex items-center gap-2 transition-all active:scale-95 disabled:opacity-50 text-sm shadow-sm shadow-emerald-500/20"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-emerald-50/50 p-6 rounded-2xl border border-emerald-100 space-y-4">
              ...
              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Pilih Marhalah (Wajib)</label>
                  <select 
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Pilih Kelas (Opsional)</label>
                  <select 
                    className="w-full p-3 border border-emerald-200 rounded-xl bg-white outline-none focus:ring-2 focus:ring-emerald-500 font-medium text-sm disabled:bg-slate-100 disabled:opacity-50"
                    value={selectedKelas}
                    onChange={(e) => setSelectedKelas(e.target.value)}
                    disabled={!selectedMarhalah}
                  >
              ...
              <button 
                ...
                className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl flex justify-center items-center gap-2 disabled:opacity-50 transition-all shadow-sm active:scale-[0.98] font-bold mt-2"
              >
              ...
              {excelData.length > 0 && (
                <div className="bg-white p-4 rounded-xl border border-blue-200 animate-in fade-in slide-in-from-bottom-2 shadow-sm">
                  <div className="flex justify-between items-center mb-3 border-b border-blue-50 pb-2">
                    <span className="text-sm font-bold text-slate-700">Preview: {excelData.length} Baris Terbaca</span>
                  ...
                  <button 
                    ...
                    disabled={isProcessing}
                    className="w-full bg-blue-700 hover:bg-blue-800 text-white py-3 rounded-xl flex justify-center items-center gap-2 disabled:opacity-50 font-bold shadow-sm transition-transform active:scale-[0.98]"
                  >
              ...
              <div className="max-h-80 overflow-auto">
                <table className="w-full text-sm text-left">
                  <thead className="bg-white sticky top-0 z-10 shadow-sm">
                    <tr>
                      <th className="p-4 font-semibold text-slate-600 border-b">NIS</th>
                      <th className="p-4 font-semibold text-slate-600 border-b">Nama Santri</th>
                      <th className="p-4 font-semibold text-slate-600 border-b">Kelas Asal</th>
                      <th className="p-4 font-semibold text-slate-600 border-b text-center">Grade<br/>Wali Kelas</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {excelData.slice(0, 50).map((row, i) => (
                      <tr key={i} className="hover:bg-slate-50">
                        <td className="p-4 font-mono text-xs text-slate-500">{row['NIS'] || row['nis']}</td>
                        <td className="p-4 font-bold text-slate-800">{row['NAMA SANTRI']}</td>
                        <td className="p-4 text-slate-600 text-sm font-medium">{row['KELAS SEKARANG']}</td>
                ...
                {excelData.length > 50 && (
                  <div className="p-4 text-center text-xs font-bold text-slate-500 bg-slate-50 border-t shadow-inner">
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100 animate-in zoom-in-95">
            <div className="p-6 text-center">
              ...
              <h3 className="text-xl font-black text-slate-800 mb-2">Konfirmasi Finalisasi</h3>
              ...
              <div className="grid grid-cols-2 gap-3 mt-4">
                <button 
                  ...
                  className="py-3 px-4 rounded-xl border-2 border-slate-200 text-slate-600 font-bold hover:bg-slate-50 transition-colors"
                >
                  Batal
                </button>
                <button 
                  onClick={handleProcessExcel}
                  className="py-3 px-4 rounded-xl bg-blue-600 text-white font-black hover:bg-blue-700 shadow-sm shadow-blue-600/30 transition-transform active:scale-95 flex items-center justify-center gap-2"
                >
                  Ya, Eksekusi
                </button>
              </div>
