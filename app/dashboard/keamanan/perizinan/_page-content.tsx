...
export default function PerizinanPage({ userRoles = [], asramaBinaan, canCreate = false, canUpdate = false, canDelete = false }: Props) {
  ...
  const [isOpenPengajuan, setIsOpenPengajuan] = useState(false)
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-20">
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-end md:items-center justify-center p-0 md:p-4 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-t-3xl md:rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in slide-in-from-bottom-5">
            ...
            <form onSubmit={handleSimpan} className="p-5 max-h-[80vh] overflow-y-auto space-y-5">
              ...
              {selectedSantri && (
                <div className="pt-2">
                  <button type="submit" className={`w-full ${isAsrama ? 'bg-purple-600 hover:bg-purple-700 shadow-purple-200' : 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-200'} text-white py-3.5 rounded-xl font-bold shadow-sm active:scale-95 transition-all flex items-center justify-center gap-2`}>
                    {isOpenEdit
      ...
      {/* --- MODAL PENGAJUAN ASRAMA --- */}
      {isOpenPengajuan && (
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-end md:items-center justify-center p-0 md:p-4 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-t-3xl md:rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in slide-in-from-bottom-5">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-amber-50">
              <div>
                <h3 className="font-bold text-slate-900 text-lg flex items-center gap-2">
                  <Clock className="w-5 h-5 text-amber-600" />
                  Pengajuan Izin dari Asrama
                </h3>
                <p className="text-xs text-slate-500 mt-0.5">
                  {pengajuanList.length} pengajuan menunggu persetujuan
                </p>
              </div>
              <button type="button" onClick={() => setIsOpenPengajuan(false)} className="p-2 bg-white hover:bg-rose-50 text-slate-400 hover:text-rose-500 rounded-full transition-colors border border-slate-200">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="max-h-[65vh] overflow-y-auto p-4">
              {pengajuanList.length === 0 ? (
                <div className="text-center py-16 flex flex-col items-center text-slate-400">
                  ...
                  <p className="font-medium">Tidak ada pengajuan yang menunggu persetujuan.</p>
                </div>
              ) : (
                <div className="space-y-3">
                      ...
                      <div className="bg-slate-50 rounded-xl p-3 text-xs text-slate-600 mb-3 space-y-1.5 border border-slate-100">
                        ...
                        {item.submitted_by_name && (
                          <p className="text-slate-400 pt-1 border-t border-slate-200">
                            Diajukan oleh: <span className="font-medium text-slate-600">{item.submitted_by_name}</span>
                          </p>
                        )}
                      </div>
            ...
            <div className="p-4 border-t bg-slate-50">
              <button type="button" onClick={() => setIsOpenPengajuan(false)} className="w-full py-2.5 border border-slate-200 rounded-xl font-bold text-slate-600 text-sm bg-white hover:bg-slate-50 transition-colors">
                Tutup
              </button>
            </div>
          </div>
