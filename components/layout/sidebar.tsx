...
import { 
  ...
  ChevronLeft, ChevronRight, Printer, ClipboardCheck, UserCheck, MapPin, Book, UserCog, RefreshCw, Moon, Stethoscope, Clock, Gavel, CreditCard, LayoutList, FileSpreadsheet, Mail
} from "lucide-react";
...
const menuGroups = [
  {
    label: "Utama",
    items: [
      { title: "Dashboard", href: "/dashboard", icon: LayoutDashboard, roles: ['admin', 'keamanan', 'sekpen', 'dewan_santri', 'pengurus_asrama', 'wali_kelas'] },
      { title: "Data Santri", href: "/dashboard/santri", icon: Users, roles: ['admin', 'keamanan', 'sekpen', 'dewan_santri', 'pengurus_asrama', 'wali_kelas'] },
    ]
  },
  {
...
export function Sidebar({ userRole = 'wali_kelas', isCollapsed, toggleSidebar, onMobileClose }: SidebarProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full text-white/90 relative">
      <button onClick={toggleSidebar} className="absolute -right-3 top-20 bg-green-700 text-white p-1 rounded-full border border-green-600 shadow-md hover:bg-green-600 transition-colors z-50 hidden md:flex">
        {isCollapsed ? <ChevronRight size={14} /> : <ChevronLeft size={14} />}
      </button>

      <div className={cn("flex items-center border-b border-white/10 shrink-0 bg-black/10 backdrop-blur-sm transition-all duration-300 overflow-hidden", isCollapsed ? "h-16 justify-center" : "h-24 px-4 gap-4")}>
        {isCollapsed ? (
        ...
        {menuGroups.map((group, idx) => {
          ...
          return (
            <div key={idx} className="animate-in fade-in slide-in-from-left-2 duration-500">
              {!isCollapsed && (
                <h3 className="text-[10px] font-bold text-green-200/60 uppercase tracking-widest mb-2 px-4 whitespace-nowrap overflow-hidden">{group.label}</h3>
              )}
              <div className="space-y-1">
                {allowedItems.map((item) => {
                  const isActive = pathname === item.href;
                  return (
                    <Link key={item.href} href={item.href} onClick={onMobileClose} title={isCollapsed ? item.title : undefined} className={cn("flex items-center rounded-lg transition-all duration-200 text-sm font-medium group relative", isCollapsed ? "justify-center p-2.5" : "space-x-3 px-4 py-2.5", isActive ? "bg-white/10 text-white shadow-lg shadow-black/5 ring-1 ring-white/20" : "text-green-100/70 hover:bg-white/5 hover:text-white")}>
                      <item.icon className={cn("transition-colors flex-shrink-0", isCollapsed ? "w-5 h-5" : "w-4 h-4", isActive ? "text-green-300" : "text-green-400/70 group-hover:text-green-300")} />
      ...
      {!isCollapsed && (
        <div className="p-4 border-t border-white/10 text-[10px] text-green-200/40 text-center shrink-0 bg-black/10 whitespace-nowrap overflow-hidden">
          ...
          <div className="mt-2 flex items-center justify-center gap-2 bg-black/20 rounded py-1 px-2">
            ...
            <button onClick={checkRole} disabled={isRefreshing} className="text-white hover:text-green-300 transition-colors p-1" title="Refresh Hak Akses">
