...
const quranArabicByLatinName = new Map(
  QURAN_SURAHS.map((surah, index) => [surah.name.toLowerCase(), QURAN_ARABIC_NAMES[index]])
)

export function getQuranSurahArabicName(surah: { number: number; name: string }) {
  return QURAN_ARABIC_NAMES[surah.number - 1] || surah.name
}

export function displayQuranSurahTitle(title: string) {
