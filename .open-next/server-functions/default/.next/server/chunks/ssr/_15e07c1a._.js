...
Read more: https://nextjs.org/docs/messages/invalid-use-server-value`),"__NEXT_ERROR_CODE",{value:"E352",enumerable:!1,configurable:!0})}}Object.defineProperty(c,"__esModule",{value:!0}),Object.defineProperty(c,"ensureServerEntryExports",{enumerable:!0,get:function(){return d}})},6846,a=>{"use strict";var b=a.i(5246),c=a.i(12259);let d=new Set(["password","password_hash","token","cookie","cookies","authorization","auth","secret"]),e=null,f=new Map;function g(a,b=500){return a.length<=b?a:`${a.slice(0,b)}...`}function h(a){return null==a?null:"string"==typeof a?g(a):"number"==typeof a||"boolean"==typeof a?a:Array.isArray(a)?a.slice(0,25).map(h):"object"==typeof a?Object.fromEntries(Object.entries(a).filter(([a])=>!d.has(a.toLowerCase())).slice(0,50).map(([a,b])=>[a,h(b)])):String(a)}function i(a){return a?{id:a.id,name:a.full_name,email:a.email,roles:a.roles}:null}async function j(){try{let a=await (0,b.headers)(),c=a.get("x-forwarded-for");return{ipAddress:a.get("cf-connecting-ip")??(c?c.split(",")[0]?.trim():null)??null,userAgent:a.get("user-agent")}}catch{return{ipAddress:null,userAgent:null}}}async function k(){e||(e=(async()=>{await (0,c.execute)(`
  ...
  `,[a]);return c.length>0?c:(0,b.query)(`
    ...
    WHERE rp.kelas_id = ?
      AND lower(trim(COALESCE(s.status_global, 'aktif'))) NOT IN ('arsip', 'alumni', 'keluar')
    ORDER BY s.nama_lengkap
  `,[a])}a.s(["HAFALAN_TYPES",0,e,"QURAN_SURAHS",0,f,"canAccessKelas",()=>q,"displayQuranSurahTitle",()=>j,"ensureGuruFeatureSchema",()=>l,"getAccessibleKelasForSession",()=>o,"getGuruIdForSession",()=>n,"getQuranSurahArabicName",()=>i,"getSantriForKelas",()=>r,"isHafalanType",()=>k])},18955,a=>{"use strict";var b=a.i(24895),c=a.i(37936),d=a.i(18558),e=a.i(6846),f=a.i(53058),g=a.i(12259),h=a.i(3038);async function i(){let a=await (0,f.getSession)();return a?(await (0,h.ensureGuruFeatureSchema)(),{kelas:await (0,h.getAccessibleKelasForSession)(a),types:h.HAFALAN_TYPES}):{kelas:[],types:h.HAFALAN_TYPES}}async function j(a,b){let c=await (0,g.queryOne)("SELECT marhalah_id FROM kelas WHERE id = ?",[a]);if(!c?.marhalah_id)return null;let d=await (0,g.queryOne)(`
    ...
    WHERE hpm.marhalah_id = ?
      ...
      AND hp.jenis = ?
    LIMIT 1
  `,[c.marhalah_id,b,b]);return d?{paketId:d.id,marhalahId:c.marhalah_id}:null}async function k(a){let b=await (0,f.getSession)();if(!b||!await (0,h.canAccessKelas)(b,a))return[];await (0,h.ensureGuruFeatureSchema)();let c=await (0,g.queryOne)("SELECT marhalah_id FROM kelas WHERE id = ?",[a]);if(!c?.marhalah_id)return[];let d=await (0,g.query)(`
    ...
    WHERE hpm.marhalah_id = ?
    GROUP BY hp.jenis
  `,[c.marhalah_id]);return h.HAFALAN_TYPES.map(a=>{let b=d.find(b=>b.jenis===a.key);return b?{...a,total_bab:b.total_bab,total_blok:b.total_blok}:null}).filter(Boolean)}async function l(a,b){let c=await (0,f.getSession)();if(!c||!await (0,h.canAccessKelas)(c,a)||!(0,h.isHafalanType)(b))return{santri:[],bab:[],progress:{}};if(await (0,h.ensureGuruFeatureSchema)(),!await j(a,b))return{santri:[],bab:[],progress:{},progressStatus:{}};let d=await (0,g.queryOne)(`
    SELECT k.marhalah_id, m.urutan AS marhalah_urutan
    FROM kelas k
    LEFT JOIN marhalah m ON m.id = k.marhalah_id
    WHERE k.id = ?
  `,[a]);if(!d?.marhalah_id)return{santri:[],bab:[],progress:{},progressStatus:{}};let e=d.marhalah_urutan??999999,[i,k]=await Promise.all([(0,h.getSantriForKelas)(a),(0,g.query)(`
    ...
    `,[d.marhalah_id,b,b,e,d.marhalah_id,b])]),l=new Map,m=[];for(let a of k)l.has(a.bab_id)||l.set(a.bab_id,{id:a.bab_id,judul:a.parent_judul?`${a.parent_judul} / ${a.judul}`:"quran"===b?(0,h.displayQuranSurahTitle)(a.judul):a.judul,urutan:a.bab_urutan,parent_id:a.parent_id,is_editable:1===a.is_editable,source_marhalah_nama:a.source_marhalah_nama,blok:[]}),a.blok_id&&(1===a.is_editable&&m.push(Number(a.blok_id)),l.get(a.bab_id).blok.push({id:a.blok_id,label:a.label,deskripsi:a.deskripsi,urutan:a.blok_urutan,is_editable:1===a.is_editable}));let n=Array.from(l.values()).filter(a=>a.blok.length>0);if(0===i.length)return{santri:i,bab:n,progress:{},progressStatus:{},editableBlokIds:m};let o=i.map(a=>a.santri_id),p=new Map(i.map(a=>[a.santri_id,a.riwayat_id])),q=o.map(()=>"?").join(","),r=await (0,g.query)(`
    ...
    WHERE hb.jenis = ?
      AND hp.santri_id IN (${q})
  `,[b,...o]),s={},t={},u={};for(let a of r){let b=p.get(a.santri_id);if(!b)continue;s[`${b}:${a.blok_id}`]=!0,t[`${b}:${a.blok_id}`]=a.status;let c=`${b}:${a.blok_id}`;u[c]=!!u[c]||a.marhalah_id===d.marhalah_id}return{santri:i,bab:n,progress:s,progressStatus:t,progressEditable:u,editableBlokIds:m}}async function m(a){let b=await (0,f.getSession)();if(!b)return{error:"Tidak terautentikasi."};if(!await (0,h.canAccessKelas)(b,a.kelasId))return{error:"Akses kelas ditolak."};if(!(0,h.isHafalanType)(a.jenis))return{error:"Jenis hafalan tidak valid."};await (0,h.ensureGuruFeatureSchema)();let c=await (0,g.queryOne)(`
  ...
  `,[a.riwayatId,a.kelasId,a.blokId,a.jenis]);if(!c)return{error:"Blok hafalan tidak tersedia untuk kelas ini."};let i=await (0,g.queryOne)("SELECT id FROM hafalan_progress WHERE blok_id = ? AND santri_id = ? AND marhalah_id = ? AND status = 'hafal'",[a.blokId,c.santri_id,c.marhalah_id]);return i?await (0,g.execute)("DELETE FROM hafalan_progress WHERE id = ?",[i.id]):await (0,g.execute)(`
    ...
    `,[(0,g.generateId)(),a.blokId,a.riwayatId,c.santri_id,a.kelasId,c.marhalah_id,await (0,h.getGuruIdForSession)(b),b.id]),await (0,e.logActivity)({actor:(0,e.actorFromSession)(b),module:"guru_hafalan",action:i?"delete":"update",fiturHref:"/dashboard/guru/hafalan",logKind:"update",entityType:"hafalan_progress",entityId:`${a.riwayatId}:${a.blokId}`,summary:i?"Membatalkan blok hafalan santri":"Menandai blok hafalan santri",details:{kelas_id:a.kelasId,jenis:a.jenis,blok_id:a.blokId}}),(0,d.revalidatePath)("/dashboard/guru/hafalan"),{success:!0,checked:!i}}async function n(a){let b=await (0,f.getSession)();if(!b)return{error:"Tidak terautentikasi."};if(!await (0,h.canAccessKelas)(b,a.kelasId))return{error:"Akses kelas ditolak."};if(!(0,h.isHafalanType)(a.jenis))return{error:"Jenis hafalan tidak valid."};await (0,h.ensureGuruFeatureSchema)();let c=await (0,g.query)(`
    SELECT hblk.id, rp.santri_id, k.marhalah_id
    FROM hafalan_blok hblk
    JOIN hafalan_bab hb ON hb.id = hblk.bab_id
    JOIN hafalan_paket_marhalah hpm ON hpm.paket_id = hb.paket_id AND hpm.jenis = hb.jenis
    ...
    WHERE k.id = ?
      ...
      AND hb.is_active = 1
      AND hblk.is_active = 1
  `,[a.riwayatId,a.kelasId,a.jenis]),i=new Set(c.map(a=>Number(a.id))),j=Array.from(new Set((a.checkedBlokIds||[]).map(Number).filter(a=>i.has(a)))),k=await (0,h.getGuruIdForSession)(b);if(0===i.size)return{error:"Belum ada blok hafalan aktif untuk kelas ini."};let l=c[0];if(!l?.santri_id||!l.marhalah_id)return{error:"Data santri atau marhalah tidak valid."};let m=Array.from(i).map(()=>"?").join(",");for(let c of(await (0,g.execute)(`DELETE FROM hafalan_progress
     WHERE santri_id = ?
       AND marhalah_id = ?
       AND blok_id IN (${m})`,[l.santri_id,l.marhalah_id,...Array.from(i)]),j)){let d=await (0,g.queryOne)("SELECT id, marhalah_id FROM hafalan_progress WHERE blok_id = ? AND santri_id = ? LIMIT 1",[c,l.santri_id]);if(d&&d.marhalah_id!==l.marhalah_id)continue;let e="proses"===a.statusByBlokId?.[String(c)]?"proses":"hafal";await (0,g.execute)(`
      INSERT INTO hafalan_progress (id, blok_id, riwayat_pendidikan_id, santri_id, kelas_id, marhalah_id, guru_id, status, tanggal_setor, updated_by, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, date('now'), ?, datetime('now'))
    `,[(0,g.generateId)(),c,a.riwayatId,l.santri_id,a.kelasId,l.marhalah_id,k,e,b.id])}return await (0,e.logActivity)({actor:(0,e.actorFromSession)(b),module:"guru_hafalan",action:"update",fiturHref:"/dashboard/guru/hafalan",logKind:"update",entityType:"hafalan_progress_batch",entityId:`${a.riwayatId}:${a.jenis}`,summary:`Menyimpan hafalan ${a.jenis} untuk ${j.length} blok`,details:{kelas_id:a.kelasId,jenis:a.jenis,total_blok:j.length}}),(0,d.revalidatePath)("/dashboard/guru/hafalan"),{success:!0,checkedBlokIds:j}}(0,a.i(13095).ensureServerEntryExports)([i,k,l,m,n]),(0,c.registerServerReference)(i,"004be1e3055b3b084890146a50e51618e26c340bbc",null),(0,c.registerServerReference)(k,"40525c24611c79c3a2f1a8a3d9195b1760697a8744",null),(0,c.registerServerReference)(l,"60820017a14fcb260591b25f235b6fccc7cfe467a9",null),(0,c.registerServerReference)(m,"40ee0164cfc62f041531da04771ee7f99754854dcd",null),(0,c.registerServerReference)(n,"40090849885fe413e3a0c157d4f4fc25e26f0bef0a",null),a.s([],22270),a.i(22270),a.s(["003144417e19712e947907ca31b6b39c8aa99a63df",()=>b.signOut,"004be1e3055b3b084890146a50e51618e26c340bbc",()=>i,"40090849885fe413e3a0c157d4f4fc25e26f0bef0a",()=>n,"40525c24611c79c3a2f1a8a3d9195b1760697a8744",()=>k,"60820017a14fcb260591b25f235b6fccc7cfe467a9",()=>l],18955)}];

//# sourceMappingURL=_15e07c1a._.js.map