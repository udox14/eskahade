...
Read more: https://nextjs.org/docs/messages/invalid-use-server-value`),"__NEXT_ERROR_CODE",{value:"E352",enumerable:!1,configurable:!0})}}Object.defineProperty(c,"__esModule",{value:!0}),Object.defineProperty(c,"ensureServerEntryExports",{enumerable:!0,get:function(){return d}})},6846,a=>{"use strict";var b=a.i(5246),c=a.i(12259);let d=new Set(["password","password_hash","token","cookie","cookies","authorization","auth","secret"]),e=null,f=new Map;function g(a,b=500){return a.length<=b?a:`${a.slice(0,b)}...`}function h(a){return null==a?null:"string"==typeof a?g(a):"number"==typeof a||"boolean"==typeof a?a:Array.isArray(a)?a.slice(0,25).map(h):"object"==typeof a?Object.fromEntries(Object.entries(a).filter(([a])=>!d.has(a.toLowerCase())).slice(0,50).map(([a,b])=>[a,h(b)])):String(a)}function i(a){return a?{id:a.id,name:a.full_name,email:a.email,roles:a.roles}:null}async function j(){try{let a=await (0,b.headers)(),c=a.get("x-forwarded-for");return{ipAddress:a.get("cf-connecting-ip")??(c?c.split(",")[0]?.trim():null)??null,userAgent:a.get("user-agent")}}catch{return{ipAddress:null,userAgent:null}}}async function k(){e||(e=(async()=>{await (0,c.execute)(`
  ...
  `,[a]);return c.length>0?c:(0,b.query)(`
    ...
    WHERE rp.kelas_id = ?
      AND lower(trim(COALESCE(s.status_global, 'aktif'))) NOT IN ('arsip', 'alumni', 'keluar')
    ORDER BY s.nama_lengkap
  `,[a])}a.s(["HAFALAN_TYPES",0,e,"QURAN_SURAHS",0,f,"canAccessKelas",()=>q,"displayQuranSurahTitle",()=>j,"ensureGuruFeatureSchema",()=>l,"getAccessibleKelasForSession",()=>o,"getGuruIdForSession",()=>n,"getQuranSurahArabicName",()=>i,"getSantriForKelas",()=>r,"isHafalanType",()=>k])},28125,a=>{"use strict";var b=a.i(24895),c=a.i(37936),d=a.i(18558),e=a.i(6846),f=a.i(53058),g=a.i(12259),h=a.i(3038),i=a.i(9343);async function j(){let a=await (0,f.getSession)();if(!a)return{kelas:[],mapel:[]};await (0,h.ensureGuruFeatureSchema)();let[b,c]=await Promise.all([(0,h.getAccessibleKelasForSession)(a),(0,i.getCachedMapelList)()]);return{kelas:b,mapel:c}}async function k(a,b){let c=await (0,f.getSession)();if(!c||!await (0,h.canAccessKelas)(c,a))return[];await (0,h.ensureGuruFeatureSchema)();let d=[a],e=b?(d.push(b),"AND nhs.mapel_id = ?"):"";return(0,g.query)(`
    ...
    WHERE nhs.kelas_id = ? ${e}
    GROUP BY nhs.id
    ORDER BY nhs.tanggal DESC, nhs.created_at DESC
  `,d)}async function l(a,b){let c=await (0,f.getSession)();if(!c||!await (0,h.canAccessKelas)(c,a))return{santri:[],nilai:{}};await (0,h.ensureGuruFeatureSchema)();let d=await (0,h.getSantriForKelas)(a);return b&&0!==d.length?{santri:d,nilai:Object.fromEntries((await (0,g.query)("SELECT riwayat_pendidikan_id, nilai FROM nilai_harian_detail WHERE sesi_id = ?",[b])).map(a=>[a.riwayat_pendidikan_id,a.nilai]))}:{santri:d,nilai:{}}}function m(a){let b=Number(a);return Number.isFinite(b)?Math.min(100,Math.max(0,Math.round(b))):0}async function n(a){let b=await (0,f.getSession)();if(!b)return{error:"Tidak terautentikasi."};if(!await (0,h.canAccessKelas)(b,a.kelasId))return{error:"Akses kelas ditolak."};await (0,h.ensureGuruFeatureSchema)();let c=String(a.namaSesi||"").trim();if(!c)return{error:"Nama sesi wajib diisi."};if(!a.mapelId)return{error:"Mapel wajib dipilih."};if(!a.tanggal)return{error:"Tanggal wajib diisi."};let i=await (0,g.queryOne)("SELECT tahun_ajaran_id FROM kelas WHERE id = ?",[a.kelasId]);if(!i)return{error:"Kelas tidak ditemukan."};let j=await (0,h.getGuruIdForSession)(b),k=a.sesiId||(0,g.generateId)(),l=a.sesiId?await (0,g.queryOne)("SELECT id FROM nilai_harian_sesi WHERE id = ? AND kelas_id = ?",[a.sesiId,a.kelasId]):null;l?await (0,g.execute)(`
      UPDATE nilai_harian_sesi
      SET mapel_id = ?, guru_id = ?, tanggal = ?, nama_sesi = ?, kkm = ?, deskripsi = ?, updated_at = datetime('now')
      WHERE id = ?
    `,[a.mapelId,j,a.tanggal,c,m(a.kkm),String(a.deskripsi||"").trim()||null,k]):await (0,g.execute)(`
      INSERT INTO nilai_harian_sesi (
        id, kelas_id, mapel_id, guru_id, tahun_ajaran_id, tanggal, nama_sesi, kkm, deskripsi, created_by
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `,[k,a.kelasId,a.mapelId,j,i.tahun_ajaran_id,a.tanggal,c,m(a.kkm),String(a.deskripsi||"").trim()||null,b.id]);let n=new Set((await (0,h.getSantriForKelas)(a.kelasId)).map(a=>a.riwayat_id)),o=(a.nilai||[]).filter(a=>n.has(a.riwayatId)).map(a=>({riwayatId:a.riwayatId,nilai:m(a.nilai)}));return o.length>0&&await (0,g.batch)(o.map(a=>({sql:`
      ...
      `,params:[(0,g.generateId)(),k,a.riwayatId,a.nilai]}))),await (0,e.logActivity)({actor:(0,e.actorFromSession)(b),module:"guru_nilai_harian",action:l?"update":"create",fiturHref:"/dashboard/guru/nilai-harian",logKind:l?"update":"create",entityType:"nilai_harian_sesi",entityId:k,entityLabel:c,summary:`Menyimpan nilai harian ${c} untuk ${o.length} santri`,details:{kelas_id:a.kelasId,mapel_id:a.mapelId,kkm:m(a.kkm)}}),(0,d.revalidatePath)("/dashboard/guru/nilai-harian"),{success:!0,sesiId:k,count:o.length}}async function o(a,b){let c=await (0,f.getSession)();if(!c||!await (0,h.canAccessKelas)(c,a))return{sesi:[],santri:[],nilai:{}};await (0,h.ensureGuruFeatureSchema)();let d=[a],e=b?(d.push(b),"AND mapel_id = ?"):"",i=await (0,g.query)(`
    SELECT id, mapel_id, tanggal, nama_sesi, kkm
    FROM nilai_harian_sesi
    WHERE kelas_id = ? ${e}
    ORDER BY tanggal ASC, created_at ASC
  `,d),j=await (0,h.getSantriForKelas)(a);if(!i.length||!j.length)return{sesi:i,santri:j,nilai:{}};let k=i.map(a=>a.id),l=k.map(()=>"?").join(","),m=await (0,g.query)(`
    SELECT sesi_id, riwayat_pendidikan_id, nilai
    FROM nilai_harian_detail
    WHERE sesi_id IN (${l})
  `,k),n={};for(let a of m)n[`${a.riwayat_pendidikan_id}:${a.sesi_id}`]=a.nilai;return{sesi:i,santri:j,nilai:n}}(0,a.i(13095).ensureServerEntryExports)([j,k,l,n,o]),(0,c.registerServerReference)(j,"0097302217a2f7267fc5fe027c0c2091d67e3659e4",null),(0,c.registerServerReference)(k,"6025216bd11d0a612fb3be97c0305a4ddc60419c56",null),(0,c.registerServerReference)(l,"603a5dabf03263fe2170fab8e010fe8cd910b470c5",null),(0,c.registerServerReference)(n,"4083b96d6ee7d78cc049724bb62b2f74d7a3674ccc",null),(0,c.registerServerReference)(o,"60c88c127f61ad4ddc307a9277ae83020628da9fa8",null),a.s([],16e3),a.i(16e3),a.s(["003144417e19712e947907ca31b6b39c8aa99a63df",()=>b.signOut,"0097302217a2f7267fc5fe027c0c2091d67e3659e4",()=>j,"4083b96d6ee7d78cc049724bb62b2f74d7a3674ccc",()=>n,"6025216bd11d0a612fb3be97c0305a4ddc60419c56",()=>k,"603a5dabf03263fe2170fab8e010fe8cd910b470c5",()=>l,"60c88c127f61ad4ddc307a9277ae83020628da9fa8",()=>o],28125)}];

//# sourceMappingURL=_d519f513._.js.map